...
class TrucksController < ApplicationController
  ...
  def index
    unless params[:id].nil?
      @stock_truck=StockTruck.find_by_id(params[:id].to_i)
    end
    unless @stock_truck.nil?
      @trucks = Truck.where("user_id = ? and stock_truck_id= ?",session[:user_id],params[:id]).order("updated_at desc").paginate(:page=>params[:page]||1,:per_page=>10)

    else
      if params[:status]
        @trucks = Truck.where("user_id = ? AND status =?",session[:user_id],params[:status]).order("updated_at desc").paginate(:page=>params[:page]||1,:per_page=>10)
      else
 ...
 if @search.fcity_code=="100000000000" && @search.tcity_code=="100000000000" then
    @trucks=Truck.order("created_at desc").paginate(:page=>params[:page]||1,:per_page=>10)
     ...
     @trucks=Truck.where("tcity_code =? AND fcity_code = ?",@search.tcity_code,@search.fcity_code).order("created_at desc").paginate(:page=>params[:page]||1,:per_page=>10)
  ...
  def part
    ...
    @trucks=Truck.where("stock_truck_id = ? AND user_id = ?", params[:stock_truck_id], session[:user_id]).paginate(:page=>params[:page]||1,:per_page=>10)
    respond_to do |format|
  ...
  def match
    ...
    @cargos=Cargo.find(:all,:conditions=>["fcity_code = ? AND tcity_code =?",@truck.fcity_code,@truck.tcity_code]).paginate(:page=>params[:page]||1,:per_page=>10)
    respond_to do |format|
  ...
  def create
    ...
    respond_to do |format|
      if @truck.save
        ...
        @truck.update_attribute(:tstatistic_id,@tstatistic.id)
        @ustatistic=Ustatistic.find_by_user_id(session[:user_id])
        total_truck=@ustatistic.total_truck || 0
        @ustatistic.update_attribute(:total_truck, total_truck+1)
        ...
        @stock_truck.update_attribute(:status,"配货")
  ...
  def confirm_chenjiao
    ...
    @quote=Quote.where("truck_id=? AND status=?",@truck.id,"请求成交").first #should be only one
    @inquery=Inquery.where("truck_id=? AND status=?",@truck.id,"请求成交").first#should be only one
