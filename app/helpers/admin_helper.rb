...
def show_cargo_trends
  ...
  (params[:day].to_i).downto(1).each do |i|
    ...
    if params[:field]
      counter=table_map[params[:table]].where(:created_at.gt => end_time,:created_at.lte=>start_time,params[:field]=>params[:value]).count
    else
    counter=table_map[params[:table]].where(:created_at.gt => end_time,:created_at.lte=>start_time).count
 end
    #counter=100
