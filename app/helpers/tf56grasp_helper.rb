    ...
    a.get('http://www.tf56.com/wshy.asp') do |page|
      page.parser().css("html body table table table table table tr").each do |tr|
        ...
        unless url.nil?
          ...
          if td_info.size==6
            one_item_huo[:cate_name]=td_info[0]|| "未知货物"
  ...
  def get_first_page_che
    ...
    a.get('http://www.tf56.com/wscy.asp') do |page|
      page.parser().css("html body table table table table table  table table tr").each do |tr|
        ...
        url=tr.css("td.hydash a").map { |link| link['href'] }
        unless url[0].nil?
          url="http://www.tf56.com/"+"TradeCenter/AspData/tCarDetail.asp?iWebBizCar="+url[0].to_s[-7..-1]
 ...
 def get_page_che_by_history
    ...
    a.get('http://www.tf56.com/wscy.asp') do |page|
      page.parser().css("html body table table table table table  table table tr").each do |tr|
        ...
        url=tr.css("td.hydash a").map { |link| link['href'] }
        unless url[0].nil?
          url="http://www.tf56.com/"+"ShowCarInfo.asp?iWebBizCarID="+url[0].to_s[-7..-1]
          one_item_che[:url]=url
          one_page_che<<one_item_che
          tr.css("td.hydash").each do |p|
            td_info<<p.content
  ...
  # puts "huo[:comments].size=#{huo[:comments].size}"
   if huo[:comments].size<15
     #  puts "find empty tf56 comments1"+huo[:comments]
     room_contact=RoomContact.where("room"=>huo[:comments].slice(0,6)).first
     huo[:comments]=room_contact.room+" "+room_contact.contact if room_contact
   #  puts "changed to "+huo[:comments]
   end
    begin
    a=Cargo.new(huo)
